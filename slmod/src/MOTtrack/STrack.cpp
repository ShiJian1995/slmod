...
double STrack::dt = 1.0;
double STrack::last_frame_stamp = -1.0;
...
void STrack::multi_predict(std::vector<STrack*> &stracks, byte_kalman::KalmanFilter &kalman_filter, 
							std::map<int, Eigen::Vector2f>& flow, double frame_stamp)
{
	...
	// }
	// else{
	// 	dt = frame_stamp - last_frame_stamp;
	// }
	// last_frame_stamp = frame_stamp;
	for (size_t i = 0; i < stracks.size(); i++)
	{
		// std::cout << "ID is " << stracks[i]->track_id << std::endl;
		if (stracks[i]->state != TrackState::Tracked)
		{
			stracks[i]->mean[7] = 0; // ？
		}

		kalman_filter.predict(stracks[i]->mean, stracks[i]->covariance, flow[stracks[i]->track_id], dt);
