...
//空的构造函数
FeatureTracker::FeatureTracker(PinholeCameraPtr _camera)
{
    // cam_mod = _cam_mod;
    camera = _camera;
    params = camera->getParameters();

    ROW = params.imageHeight();
    COL = params.imageWidth();
}
...
void FeatureTracker::rejectWithF()
{
    if (forw_pts.size() >= 8)
    {
        ...
        for (unsigned int i = 0; i < cur_pts.size(); i++)
        {
            ...
            camera->liftProjective(Eigen::Vector2d(forw_pts[i].x, forw_pts[i].y), tmp_p);
            tmp_p.x() = FOCAL_LENGTH * tmp_p.x() / tmp_p.z() + COL / 2.0;
            tmp_p.y() = FOCAL_LENGTH * tmp_p.y() / tmp_p.z() + ROW / 2.0;
