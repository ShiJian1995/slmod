...
ImuProcess::ImuProcess() : b_first_frame_( true ), imu_need_init_( true ), last_imu_( nullptr ), start_timestamp_( -1 )
{
    ...
    acc_s_last = Eigen::Vector3d::Zero();
...
void ImuProcess::Process(const LIMeasureGroup &meas, StatesGroup &state, PointCloudXYZINormal::Ptr pcl_un_){
    ...
    if ( imu_need_init_ )
    {
        ...
        if ( init_iter_num > MAX_INI_COUNT )
        {
            ...
            ROS_INFO(
                ...
                state.gravity[ 0 ], state.gravity[ 1 ], state.gravity[ 2 ], state.bias_g[ 0 ], state.bias_g[ 1 ], state.bias_g[ 2 ], cov_acc[ 0 ],
...
void ImuProcess::lic_point_cloud_undistort( const LIMeasureGroup &meas, const StatesGroup &_state_inout, PointCloudXYZINormal &pcl_out )
{
    StatesGroup state_inout = _state_inout;
    auto        v_imu = meas.imu;
    v_imu.push_front( last_imu_ ); // 加了上一时刻的IMU数据
    const double &imu_end_time = v_imu.back()->header.stamp.toSec();
    const double &pcl_beg_time = meas.lidar_beg_time;
    /*** sort point clouds by offset time ***/
    pcl_out = *( meas.lidar );
    std::sort( pcl_out.points.begin(), pcl_out.points.end(), time_list ); // 按照时刻进行升序排列
    ...
    */
    ...
    {
        auto &&head = *( it_imu ); // 右值引用
        auto &&tail = *( it_imu + 1 );

        // 两次IMU数据取平均, 这样的操作是否合理？
...
// fout<<head->header.stamp.toSec()<<" "<<angvel_avr.transpose()<<" "<<acc_avr.transpose()<<std::endl;
        ...
        acc_s_last = acc_imu;
        double &&offs_t = tail->header.stamp.toSec() - pcl_beg_time; // 相对于激光雷达起始点的时间偏差
    ...
    Eigen::Vector3d pos_liD_e = state_inout.pos_end + state_inout.rot_end * Lidar_offset_to_IMU; // 激光雷达在参考坐标系的位置
    ...
    for ( auto it_kp = IMU_pose.end() - 1; it_kp != IMU_pose.begin(); it_kp-- )
    {
        ...
        for ( ; it_pcl->curvature / double( 1000 ) > head->offset_time; it_pcl-- )
        {
            ...
            // 这里应该也是一个变换关系，使用矩阵连乘
            Eigen::Vector3d P_compensate = state_inout.rot_end.transpose() * ( R_i * P_i + T_ei ); // 这个地方的去畸变是怎么回事？
            ...
            if ( it_pcl == pcl_out.points.begin() ) // 一直转换到起始点
                break;
        }
    }
}

void ImuProcess::lic_state_propagate( const LIMeasureGroup &meas, StatesGroup &state_inout )
{
