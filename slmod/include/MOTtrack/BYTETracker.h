...
class Rgbmap_object_tracker;
class BYTETracker
{
public:
	BYTETracker(int frame_rate = 15, int track_buffer = 30);
	~BYTETracker();

	// std::vector<STrack> update(const std::vector<Object>& objects);
	// 重载
	std::pair<double, std::vector<STrack>> update(const std::vector<Object>& objects, 
...
private:
	...
	std::vector<STrack> tracked_stracks; // 跟踪的目标
